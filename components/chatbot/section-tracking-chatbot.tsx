...
function SectionTrackingChatbotComponent() {
  ...
  useEffect(() => {
    setMounted(true);
    console.log("Section-tracking Chatbot mounted!");
  }, []);

  // Track scroll position and detect current section
  useEffect(() => {
    if (!mounted) return;
    ...
    const handleScroll = () => {
      ...
      let foundSection = sections[0]; // Default to first section
      ...
      for (const sectionInfo of sections) {
        ...
        if (element) {
          ...
          console.log(`Section ${sectionInfo.id}: top=${absoluteTop}, bottom=${absoluteBottom}, scrollCenter=${scrollCenter}`);
          
          // Check if scroll center is within this section
          if (scrollCenter >= absoluteTop && scrollCenter <= absoluteBottom) {
            foundSection = sectionInfo;
            break;
          }
        }
      }
      ...
      if (foundSection.id !== currentSection.id) {
        console.log("Switching to section:", foundSection.name);
    ...
    // Check for sections periodically in case they're added dynamically
  ...
  const calculatePosition = () => {
    ...
    // Position between 20% and 80% of viewport height
...
export default dynamic(() => Promise.resolve(SectionTrackingChatbotComponent), {
  ssr: false
});