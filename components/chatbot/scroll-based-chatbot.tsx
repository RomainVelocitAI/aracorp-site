...
interface SectionInfo {
  ...
  botResponse: string;
  scrollStart: number; // Percentage where section starts
  scrollEnd: number;   // Percentage where section ends
}

// Define sections with scroll percentage ranges
...
function ScrollBasedChatbotComponent() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Find current section based on scroll percentage
      let newSection = sections[sections.length - 1]; // Default to last section
      
      for (const section of sections) {
        if (percent >= section.scrollStart && percent < section.scrollEnd) {
          newSection = section;
          break;
        }
      }
      ...
      if (newSection.id !== currentSection.id) {
        console.log(`Section changed to: ${newSection.name} at ${percent.toFixed(1)}%`);
  ...
  const calculatePosition = () => {
    ...
    // Move between 30% and 70% of viewport based on scroll
    return Math.round(vh * 0.3 + (vh * 0.4 * scrollPercent / 100));
  };
        ...
        >
          ...
          <div style={{
            ...
            background: `linear-gradient(135deg, ${
              currentSection.name === 'Digiqo' ? '#3b82f6' : 
              currentSection.name === 'RunCall' ? '#10b981' : 
              currentSection.name === 'Nos Filiales' ? '#f59e0b' :
              '#8b5cf6'
            } 0%, #ec4899 100%)`,
            borderRadius: '50%',
            opacity: 0.3,
            ...
            {sections.map(section => (
              <button
                ...
                onClick={() => {
                  ...
                  const scrollTarget = (section.scrollStart + section.scrollEnd) / 2;
                  const windowHeight = window.innerHeight;
                  const documentHeight = document.documentElement.scrollHeight;
                  const maxScroll = documentHeight - windowHeight;
...
export default dynamic(() => Promise.resolve(ScrollBasedChatbotComponent), {
  ssr: false
});